...
if (isset($data->username) && isset($data->project_name) && isset($data->description)) {
    ...
    // Query to update the todo status to pending (0)
    $query = "UPDATE todos SET completed = 0 WHERE username = ? AND project_name = ? AND description = ?";
    ...
    if ($stmt = $con->prepare($query)) {
        ...
        // Execute the query
        if ($stmt->execute()) {
            echo json_encode(["status" => "success", "message" => "Todo marked as pending"]);
        } else {
            echo json_encode(["status" => "error", "message" => "Failed to mark todo as pending"]);
        }
